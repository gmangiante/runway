...
const { data } = $(await useFetch<Dataset>(`http://runway-demo.herokuapp.com/api/datasets/${props.dataset_id}`))
const analysisFetch = $(await useFetch<DatasetAnalysis>(`http://runway-demo.herokuapp.com/api/datasets/analyze/${props.dataset_id}`))
...
const submitForm = async (e: Event) => {
    ...
    const newModelFetch =
        await useFetch<{ new_model_id: number }>('http://runway-demo.herokuapp.com/api/models/', { method: 'POST', body: JSON.stringify(newModel) })
  ...
  async () => {
    if (router.currentRoute.value.query['dup']) {
      ...
      const existingModelFetch =
        await useFetch<Model>(`http://runway-demo.herokuapp.com/api/models/${existing_model_id}`)
